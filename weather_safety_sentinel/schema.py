...
class WeatherReport(BaseModel):
    ...
    current_units: CurrentUnits = Field(description="The units for the current weather data.")

    class Current(BaseModel):
        time: str = Field(description="The timestamp of the current weather data.")
        interval: int = Field(description="The interval in seconds for the current weather data.")
        temperature_2m: float = Field(description="The current temperature at 2 meters in Celsius.")
        wind_speed_10m: float = Field(description="The current wind speed at 10 meters in km/h.")
